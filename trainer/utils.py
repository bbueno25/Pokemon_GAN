...
pp = PrettyPrinter()
get_stddev = lambda x, k_h, k_w: 1 / math_sqrt(k_w * k_h * x.get_shape()[-1])
...
def center_crop(x, crop_h, crop_w, resize_h=64, resize_w=64):
    ...
    return misc.imresize(x[j: j + crop_h, i: i + crop_w], [resize_h, resize_w])
...
def visualize(sess, dcgan, config, option):
    image_frame_dim = int(math_ceil(config.batch_size**0.5))
    if option == 0:
        ...
        values = np.arange(0, 1, 1.0 / config.batch_size)
        for idx in [np.random.randint(0, 99) for _ in xrange(100)]:
            print(" [*] %d" % idx)
